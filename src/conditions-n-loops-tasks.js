...
function convertToRomanNumerals(num) {
  ...
  if (a > 0) {
    let n = 1;
    while (n <= a) {
      result += numbToRoman['10'];
      n += 1;
    }
  }
  const b = num % 10;
  if (b > 0) {
    result += numbToRoman[String(b)];
  }
  return result;
}
...
function sortByAsc(arr) {
  ...
  const baseElem = arr[arr.length - 1];
  const left = [];
  const right = [];
  let indLeft = 0;
  let indRight = 0;
  ...
  for (let i = 0; i < arr.length - 1; i += 1) {
    if (arr[i] < baseElem) {
      ...
      right[indRight] = arr[i];
      indRight += 1;
    }
  }
  const result = [...sortByAsc(left), baseElem, ...sortByAsc(right)];
...
function getNearestBigger(number) {
  ...
  for (let i = 0; i < strFromNumber.length; i += 1) {
    arr[i] = +strFromNumber[i];
  }
